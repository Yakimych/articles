...
At this point we can use the `reasonSum` function from JavaScript of TypeScript &mdash; let's call it from our React component:
...
Adding ReasonML to an existing React codebase can be done in a matter of minutes. After this initial setup, it becomes possible to write logic in ReasonML or OCaml and use it in existing React components. This approach is a alternative to jumping straight into [ReasonReact](https://reasonml.github.io/reason-react/) (in case it seems too big of a step). The source code is available on [GitHub](TODO).

The same approach can be used for adding ReasonML to a [Vue.js application](https://github.com/Yakimych/reason-in-typescript/blob/master/reason-in-vue/README.md), or pretty much [any other JavaScript application](https://github.com/Yakimych/reason-in-typescript/blob/master/basic-javascript/README.md).
